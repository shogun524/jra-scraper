...
from scrape_race_result import fetch_race_page, parse_race_result
...
def diagnose(race_id: str):
    ...
    print(f"パース結果: {len(rows)}頭分")
    for r in rows[:3]:
        print(json.dumps(r, ensure_ascii=False, indent=2))
    if not rows:
        print("!! 0頭でした。scrape_race_result.py の COLUMN_MAP / テーブル抽出ロジックを見直してください。")
        print("!! html[:2000] を見て構造を確認:")
        print(html[:2000])
