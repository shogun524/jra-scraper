...
def diagnose(date_str: str):
    """1日分だけ取得して中身を確認する(0件だった原因の切り分け用)"""
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page(user_agent=UA)
        url = RACE_LIST_URL.format(date=date_str)
        ...
        print(f"最終的なURL(リダイレクトされていないか確認): {page.url}")
        page.wait_for_timeout(2500)
        title = page.title()
        html = page.content()
        page.screenshot(path="data/diagnose_race_ids_screenshot.png", full_page=True)
        browser.close()

    print(f"ページタイトル: {title}")
    print(f"HTML長: {len(html)} 文字")
    print("data/diagnose_race_ids_screenshot.png にスクリーンショットを保存しました")
    ids = sorted(set(re.findall(r"race_id=(\d{12})", html)))
    print(f"見つかったrace_id(全場): {len(ids)}件  {ids[:5]}")
    for kw in ["エラー", "見つかりません", "ログイン", "会員", "しばらくお待ち", "アクセスが集中", "確認してください"]:
        if kw in html:
            pos = html.find(kw)
